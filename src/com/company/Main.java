        ...
        while (true) {
            ...
            try (SocketChannel socketChannel = serverChannel.accept()) {
                ...
                while (socketChannel.isConnected()) {
                    ...
                    inputBuffer.clear();
                    // код для очистки от пробелов
                    String withoutBlanks = msg.replaceAll(" ", "");

                    //System.out.println("Получено сообщение от клиента: " + msg);
                    // отправляем сообщение клиента назад с пометкой ЭХО
                    socketChannel.write(ByteBuffer.wrap(("Пробелы удалены сервером : " +
                            withoutBlanks).getBytes(StandardCharsets.UTF_8)));
                }
